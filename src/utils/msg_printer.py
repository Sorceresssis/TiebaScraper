...
def generate_affiliation_str(affiliations: list):
    if len(affiliations) % 2 != 0:
        raise ValueError(
            "The affiliations list must contain an even number of elements."
        )

    formatted_pairs = [
        f"{affiliations[i]}:{affiliations[i + 1]}"
        for i in range(0, len(affiliations), 2)
    ]
...
class MsgPrinter:
    ...
    @classmethod
    def print_error(
            cls, msg: str = "", label: str | None = None, affiliations: list = []
    ):
        print(
            f"{PrintColor.RED}ERROR{PrintColor.RESET}",
            (
                f" - {PrintColor.BLUE}{label}{PrintColor.RESET}"
    ...
    def print_tip(cls, msg: str = "", affiliations: list = []):
        print(
            ...
            (
                f" {generate_affiliation_str(affiliations)} - "
                if len(affiliations)
                else " - "
            ),
            f"{msg}" if len(msg) else "",
